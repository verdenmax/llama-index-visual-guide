...
LESSON_32 = (
    c.pipeline("answer")
    + c.lead(L(
        "L26 已经把 RAG 从“写死的链”升级成“会决策的循环”——单个 agent + 一条最小 workflow。但任务一旦要"
        "跨好几种<strong>能力</strong>：先<strong>查</strong>资料、再<strong>写</strong>答复、最后<strong>核</strong>事实，"
        "把工具和规则<strong>全塞进一个 agent</strong>就会乱：工具一多越容易<strong>选错</strong>，system_prompt 越写越长越"
        "<strong>打架</strong>，出错还分不清是哪一环。这一课就把“一个全能选手”升级成“一支有分工、会<strong>交接</strong>"
        "的小队”，并让它的<strong>行进路线</strong>变成可检查的代码。",
        "L26 already upgraded RAG from a “hard-wired chain” to a “deciding loop” — one agent plus a minimal workflow. "
        "But once the task spans several <strong>capabilities</strong> — first <strong>search</strong> for facts, then "
        "<strong>write</strong> the reply, finally <strong>check</strong> them — cramming all those tools and rules "
        "<strong>into one agent</strong> gets messy: with many tools it more easily <strong>picks wrong</strong>, its "
        "system prompt grows long and <strong>self-contradictory</strong>, and on a failure you can't tell which part "
        "broke. This lesson upgrades “one all-round player” into “a small team with clear roles that <strong>hands "
        "off</strong>”, and makes that team's <strong>route</strong> inspectable code.",
    ))
    + d.compare2(
    ...
    + c.section(
        ...
        L(
            "<strong>这一课往前走两步</strong>：① 把一个全能 agent <strong>拆成几个各管一段的专精 agent</strong>，用 "
            "<strong>handoff（交接）</strong>把任务往下传，由 <code>AgentWorkflow</code> 编排；② 再往底层落到 "
            "<strong>workflow</strong>，用 <code>@step</code> + 事件把<strong>分支 / 循环</strong>写成<strong>显式、可"
            "单测</strong>的控制流。下面分节展开，先看为什么单 agent 不够。",
            "<strong>This lesson moves two steps</strong>: (1) split one do-everything agent into <strong>several "
            "specialized agents, each owning a slice</strong>, and use <strong>handoff</strong> to pass the task on, "
            "orchestrated by <code>AgentWorkflow</code>; (2) drop to a <strong>workflow</strong> underneath, where "
            "<code>@step</code> + events make <strong>branches / loops</strong> into <strong>explicit, testable</strong> "
            "control flow. The sections unpack these — starting with why one agent isn't enough.",
        ),
        L(
            ...
            "whether it “searched wrong” or “wrote off-track”.",
        ),
        L(
            "解法是<strong>分工</strong>：把一个全能 agent 拆成几个<strong>各管一段</strong>的专精 "
            ...
            "连同上下文</strong>交给下一个更合适的 agent，像接力赛传棒（查→写→核三个角色就能串成一条线）。",
            "The fix is <strong>division of labor</strong>: split one "
            ...
            "next, more suitable agent, like a relay baton (search→write→check threads three roles into one line).",
        ),
        c.alert(L(
            "<strong>要点：handoff（交接）不是“关掉某个工具”，也不是“换个更大的模型”</strong>，而是把<strong>任务移交给队友</strong>。",
            "<strong>The point: a handoff is not “turning off a tool” and not “switching to a bigger model”</strong> — "
            "it is <strong>handing the task to a teammate</strong>.",
        ), kind="note"),
    )
    + c.section(
        ...
        L(
            ...
            "<strong>entry</strong> (whoever takes the task first).",
        ),
        L(
            "运行 <code>await workflow.run(user_msg=...)</code>，root agent 先处理，需要时<strong>调用"
            ...
            "Calling <code>await workflow.run(user_msg=...)</code>, "
    ...
    + c.section(
        ...
        L(
            ...
            "这种<strong>循环</strong>——就可以<strong>落到裸 <code>Workflow</code></strong>，亲手把流程画清楚。",
            ...
            "can <strong>drop to a raw <code>Workflow</code></strong> and draw the flow by hand.",
        ),
        L(
            "写法承接 L26 的"
            ...
            "into the graph</strong>.",
        ),
        c.alert(L(
            "于是控制流不再藏在一长串 <code>if/else</code> 里，而是<strong>变成显式的"
            ...
            "So control flow no longer hides inside a long <code>if/else</code>; it "
    ...
    + c.section(
        ...
        L(
            ...
            "it handed off that way”, which turns L23's <strong>tracing from a nice-to-have into a must-have</strong>.",
        ),
        c.alert(L(
            "<strong>越多 agent、越自由越强，但每多一次 handoff，就多一次 LLM 决策</strong>——更慢、更贵、更难调。",
            "<strong>More agents and more freedom make the system more capable, but each extra handoff is one more LLM "
            "decision</strong> — slower, pricier, harder to debug.",
        ), kind="warn"),
        L(
            "所以延续 L26 的那把尺子，按需选<strong>最低自主度</strong>：<strong>固定管道 → 单 agent → "
    ...
    + c.design_highlight(
        i18n.render(L(
            ...
            "observable, testable pieces</strong>.",
        ))
        + i18n.render(L(
            "但更强从来不是免费的——<strong>多一个 agent 就多一次 LLM 决策、多一处可能跑偏</strong>，"
            ...
            "agent、甚至一条固定管道，往往又快又稳。",
            "But more power is never free — <strong>each extra agent is another LLM "
        ...
        + i18n.render(L(
            "还有一道绕不开的关：当这支自由的小队要去<strong>删数据、下订单、发邮件</strong>"
            "这类<strong>高风险动作</strong>时，光靠它们自己拍板就太冒险了——得有人能<strong>在回路里</strong>喊停、点头确认。那正是"
            "下一课 <strong>L33 人在回路（HITL）</strong>要补上的“刹车”。",
            "And one gate is unavoidable: when this free-roaming team goes to <strong>delete data, place "
...
LESSON_33 = (
    ...
    + c.lead(L(
        ...
        "而是<strong>只在不可逆 / 高风险的那一步加闸</strong>，其余照常自动。",
        ...
        "step</strong>, leaving the rest automatic.",
    ))
    + d.compare2(
    ...
    + c.section(
        ...
        L(
            ...
            "（读文档、起草文字、再查一次资料）来说，选错的代价很低：<strong>重试一次</strong>就好，没人受伤。可<strong>"
            "另一类动作</strong>会<strong>越过边界、落到真实世界</strong>，代价就完全不同了——下面这一类，正是要盯紧的。",
            ...
            "wrong pick is cheap: just <strong>retry</strong>, no one gets hurt. But <strong>another class of "
            "action</strong> <strong>crosses the boundary into the real world</strong>, where the stakes are entirely "
            "different — and that class is the one to watch.",
        ),
        c.alert(L(
            "<strong>删库、转账、下单、对外发邮件这类动作不可逆</strong>——一旦做错就收不回来，<strong>不能让模型独自拍板</strong>。",
            "<strong>Dropping a database, transferring money, ordering, sending an outbound email are irreversible"
            "</strong> — once wrong you can't take them back, so <strong>the model must not decide alone</strong>.",
        ), kind="warn"),
        L(
            "<strong>正因如此</strong>，HITL 的思路很简单：在<strong>这一步真正落地前</strong>"
            ...
            "留下的那道“刹车”。",
            "<strong>That is exactly why</strong> HITL's idea is simple: insert a human <strong>right before such a step "
            "actually lands</strong> and let them give the <strong>final approval</strong>. But a human isn't free — "
            "every confirmation <strong>adds latency and interrupts someone</strong> — so you reserve it for the "
            "<strong>costliest, least reversible</strong> actions and keep everything else automatic. This is the very "
            "“brake” the last lesson left off on.",
        ),
    )
    + c.section(
        ...
        L(
            "<strong>② 调用方侧——回灌</strong>：发起 workflow 的人正用 "
            ...
            "或中止</strong>。",
            "<strong>(2) Caller side — inject back</strong>: whoever launched the workflow is "
            ...
            "<strong>resumes or aborts</strong> accordingly.",
        ),
        c.alert(L(
            "一句话——这对“出—回”就是 HITL 的全部：<strong>workflow 返回 <code>InputRequiredEvent</code> 挂起</strong>、"
            "把 <code>prefix</code> 问题带出去；<strong>调用方流式取到后回 <code>HumanResponseEvent</code> 恢复</strong>、"
            "把 <code>response</code> 决定带回来。",
            "In a line — this “out-and-back” pair is all of HITL: <strong>the workflow returns "
            "<code>InputRequiredEvent</code> to pause</strong>, carrying the <code>prefix</code> question out; "
            "<strong>the caller streams it and sends back <code>HumanResponseEvent</code> to resume</strong>, carrying "
            "the <code>response</code> decision back.",
        ), kind="note"),
    )
    + c.code(
    ...
    + c.section(
        ...
        L(
            ...
            "裁决；关键是设<strong>阈值</strong>——只有<strong>置信度低于线</strong>才弹给人，高置信的照常自动答。",
            ...
            "human only <strong>below the line</strong>, answer high-confidence cases automatically.",
        ),
        c.alert(L(
            "<strong>只在该加闸处加闸</strong>：给低置信度<strong>设阈值</strong>、给工具<strong>分级</strong>，只把"
            "<strong>极少数</strong>不可逆 / 高风险 / 低置信的点交给人——别让 HITL 变成“凡事都问”。",
            "<strong>Gate only where a gate belongs</strong>: set a <strong>threshold</strong> for low confidence and "
            "<strong>grade</strong> your tools, handing humans only the <strong>very few</strong> irreversible / "
            "high-risk / low-confidence points — so HITL never becomes “ask about everything”.",
        ), kind="tip"),
        L(
            "<strong>③ 敏感工具调用前确认</strong>：给 agent 的工具<strong>分级</strong>——把 "
            ...
            "<strong>(3) Confirm before sensitive tool calls</strong>: <strong>grade"
    ...
    + c.design_highlight(
        i18n.render(L(
            ...
            "<strong>拖垮</strong>、把人<strong>烦走</strong>。",
            ...
            "the incident or <strong>grind throughput</strong> and <strong>annoy the human away</strong>.",
        ))
        + i18n.render(L(
            "把它放进全书的脉络看，这是<strong>三层纵深</strong>的最后一层："
...
LESSON_34 = (
    ...
    + c.lead(L(
        ...
        "<strong>能跑一次就算赢</strong>。可真正交付时，它得变成一个 <strong>7×24 营业的服务</strong>：通过 HTTP "
        "同时接很多人、要稳要快。从原型到服务，<strong>最该避免的一件事</strong>就是——<strong>每来一个请求就重建"
        "索引</strong>：建库本是<strong>一次性</strong>的重活，摊进<strong>每一次查询</strong>就又慢又浪费、并发一上来"
        "就垮。正解只有一句：<strong>索引在启动时一次加载、常驻内存，查询引擎复用</strong>，每个请求只跑「检索+合成」"
        "这段轻活。",
        ...
        "from prototype to service, the <strong>one thing to avoid most</strong> is <strong>rebuilding the index on "
        "every request</strong>: building the library is a <strong>one-time</strong> heavy job, and smearing it across "
        "<strong>every query</strong> is slow, wasteful, and collapses under concurrency. The fix is one sentence: "
        "<strong>load the index once at startup, keep it resident in memory, and reuse the query engine</strong>, so "
        "each request runs only the light “retrieve + synthesize” part.",
    ))
    + d.compare2(
    ...
    + c.section(
        ...
        L(
            "这时一个在 notebook 里<strong>看不出问题</strong>的写法会变成致命伤——<strong>把建索引"
            ...
            "吃垮</strong>。",
            "Here a pattern that <strong>looks harmless in a notebook</strong> turns fatal — "
            ...
            "<strong>crushed by repeated rebuilds</strong> the moment concurrency rises.",
        ),
        c.alert(L(
            "<strong>重活留在启动，请求里只剩轻活</strong>：服务启动时一次性把索引<strong>加载进内存</strong>、得到一个"
            "<strong>常驻引擎</strong>，之后每个请求只复用它跑「检索 + 合成」——<strong>别每请求重建</strong>。",
            "<strong>Keep the heavy work at startup, leave only light work per request</strong>: at startup load the index "
            "into memory <strong>once</strong> into a <strong>resident engine</strong>, then every request just reuses it "
            "for “retrieve + synthesize” — <strong>never rebuild per request</strong>.",
        ), kind="tip"),
        L(
            "<strong>顺着这条线，后面三节各补一块</strong>：用 <strong>FastAPI</strong> + <code>async</code> 让「轻活」"
            "并发跑得更稳（②），用 <strong>persist / <code>load_index_from_storage</code></strong> 让「启动加载」便宜"
            "到秒级（③），再用 <strong>llama-deploy / create-llama</strong> 把多个这样的服务<strong>编排起来、快速起"
            "项目</strong>（④）。",
            "<strong>From here, the next three sections each add a piece</strong>: <strong>FastAPI</strong> + "
            "<code>async</code> makes the “light work” run concurrently and stably (②), <strong>persist / "
            "<code>load_index_from_storage</code></strong> makes “startup loading” cheap down to seconds (③), and "
            "<strong>llama-deploy / create-llama</strong> <strong>orchestrate many such services and scaffold a project "
            "fast</strong> (④).",
        ),
    )
    + c.code(
    ...
    + c.section(
        ...
        L(
            ...
            "它<strong>原生 async</strong>，和我们 L24 学的 <code>aquery</code> <strong>天生一对</strong>。",
            ...
            "match</strong> for the <code>aquery</code> we learned in L24.",
        ),
        L(
            "为什么要 "
            ...
            "「异步/批」）。",
            "Why async? Because most of a RAG query's time "
            ...
            "— on the same machine, concurrent throughput jumps a level (L24's “async / batch”).",
        ),
        L(
            "另一半是<strong>体验</strong>：用<strong>流式</strong>（承 L24）让答案<strong>逐 token 边生成"
            "边吐</strong>，用户几百毫秒就看到第一个字，而不是干等整段生成完——<strong>首 token 延迟</strong>骤降，总"
            "时长几乎不变。要记牢 L24 的那句话：<strong>流式优化的是「首字延迟」与体验，不是总耗时</strong>。",
            "The other half is "
        ...
        c.alert(L(
            "<strong>分清边界</strong>：<code>aquery</code> 是 <strong>core 的真方法</strong>（直接收一个字符串问题）；"
            "<code>FastAPI</code> / <code>uvicorn</code> 是<strong>外部工具</strong>，只是把这个 core 能力<strong>暴露成 "
            "HTTP</strong>。",
            "<strong>Keep the boundary clear</strong>: <code>aquery</code> is a <strong>real core "
    ...
    + c.section(
        ...
        L(
            ...
            "——<strong>部署一次等几分钟、还白烧 embedding 钱</strong>。",
            ...
            "deploy and wasted embedding spend</strong>.",
        ),
        c.alert(L(
            "<strong>persist 落盘一次、load 秒级读回</strong>：索引<strong>建一次、写一次盘</strong>，之后每次启动 / "
            "扩容都<strong>读回、别重建</strong>。",
            "<strong>persist once, load back in seconds</strong>: build the index and <strong>write it to disk once"
            "</strong>, then every startup / scale-out <strong>reads it back instead of rebuilding</strong>.",
        ), kind="key"),
        L(
            "L11 的<strong>持久化</strong>正是为此：<strong>"
            ...
            "code① 启动那一行在做的事。",
            "L11's <strong>persistence</strong> exists for this: "
            ...
            "the startup line in code① above does.",
        ),
        L(
            "这两个符号都是<strong>实打实的 core</strong>（<code>StorageContext</code> 来自 "
            "<code>storage_context.py</code>、<code>load_index_from_storage</code> 来自 <code>indices/loading.py</code>），"
            "不依赖任何外部框架。",
            "Both symbols are <strong>genuine core</strong> "
            "(<code>StorageContext</code> from <code>storage_context.py</code>, <code>load_index_from_storage</code> from "
            "<code>indices/loading.py</code>), depending on no external framework.",
        ),
        L(
            "<strong>再往规模化看一步</strong>：当你跑<strong>多个副本</strong>（多进程 / 多机水平"
            ...
            "数据也只有一处真相。",
            "<strong>One step toward scale</strong>: "
    ...
    + c.section(
        ...
        L(
            ...
            "“objects in one process” to “a set of distributed services”</strong>.",
        ),
        L(
            ...
            "的样板，<strong>几分钟就能跑起一个能问答的网站</strong>。",
            "<strong>(2) <code>create-llama</code>"
        ...
        L(
            "<strong>务必记住边界</strong>：<code>llama-deploy"
    ...
    + c.analogy(
        i18n.render(L(
            ...
            "<strong>常年不打烊</strong>、要<strong>同时接待很多客人</strong>、上菜要<strong>快</strong>。",
            ...
            "close</strong>, <strong>serve many customers at once</strong>, and bring food out <strong>fast</strong>.",
        ))
        + i18n.render(L(
            "于是你不会"
    ...
    + c.design_highlight(
        i18n.render(L(
            ...
            "that is the single, fatal step between a notebook and a service.",
        ))
        + i18n.render(L(
            "想清楚这一点，剩下的拼图你<strong>早就备齐了</strong>：L24 的 "
            ...
            "从第 1 课搭到第 33 课</strong>的那条 RAG 管线。",
            "Get that, and the rest of the puzzle you "
            ...
            "the core still being the RAG pipeline you <strong>built from L1 through L33</strong>.",
        ))
        + i18n.render(L(
            "而「上线」从来不是终点：服务跑起来之后，质量靠全书攒下的那套"
...
LESSON_35 = (
    c.pipeline("embed")
    + c.lead(L(
        "全书<strong>最后一课</strong>，RAG 的零件你都见过了——只有最底层那个零件，我们<strong>一直原样拿来用"
        "</strong>：<strong>embedding 模型</strong>。它把文字变成向量、决定“谁和谁相近”，是检索的<strong>地基</strong>。"
        "通用 embedding（如 <code>bge</code>）在<strong>大众语料</strong>上训练，泛泛够用；可一进<strong>专业领域</strong>"
        "——医疗、法律、公司黑话——就常<strong>语义错位</strong>：本该相近的概念<strong>离得老远</strong>，"
        "<strong>召回不到位</strong>，后面 rerank、合成再好也救不回。这一课讲<strong>最后一招</strong>：用<strong>自家文档"
        "</strong>造 <strong>(问题, 相关块)</strong> 对、<strong>微调 embedding</strong>，把领域语义<strong>校准</strong>进"
        "向量空间。",
        "At the book's <strong>final lesson</strong>, you've met the parts of RAG — yet one foundational part we kept "
        "<strong>using as-is</strong> throughout: the <strong>embedding model</strong>. It turns text into vectors and "
        "decides “who is near whom” — the <strong>bedrock</strong> of retrieval. Generic embeddings (e.g. "
        "<code>bge</code>) train on <strong>broad corpora</strong> and serve general cases well; but step into a "
        "<strong>specialized domain</strong> — medicine, law, in-house slang — and they often <strong>misalign</strong>: "
        "concepts that should be near sit <strong>far apart</strong>, retrieval "
        "<strong>under-recalls</strong>, and no downstream rerank or synthesis can recover it. This lesson covers the "
        "<strong>last resort</strong>: use <strong>your own documents</strong> to build <strong>(question, "
        "relevant-chunk)</strong> pairs and <strong>fine-tune the embedding</strong>, calibrating domain semantics into "
        "the vector space.",
    ))
    + d.compare2(
    ...
    + c.section(
        ...
        L(
            "<strong>动手前先记两条前提</strong>：① 微调<strong>成本高、易过拟合</strong>，该是<strong>穷尽更便宜手段"
            "之后</strong>的最后一步、不是第一反应（详见 ④）；② <code>llama-index-finetuning</code> <strong>在 core 之外"
            "</strong>（<code>pip install llama-index-finetuning</code>），本课代码<strong>是示例</strong>，重点在"
            "<strong>思路与判断</strong>、不在 API 细节。",
            "<strong>Two caveats before diving in</strong>: (1) fine-tuning is <strong>costly and overfit-prone</strong>, "
            "a <strong>last step after exhausting cheaper means</strong> rather than a first reflex (see ④); (2) "
            "<code>llama-index-finetuning</code> <strong>lives outside core</strong> (<code>pip install "
            "llama-index-finetuning</code>); the code here is <strong>illustrative</strong> — the point is the "
            "<strong>reasoning and judgment</strong>, not API details.",
        ),
        L(
            ...
            "相近</strong>而<strong>排不进 top-k</strong>——召回阶段就把答案漏在了门外。",
            ...
            "so retrieval leaves the answer outside the door.",
        ),
        c.alert(L(
            "<strong>embedding 是检索的地基</strong>：召回阶段漏掉的块，再聪明的 rerank / 合成也<strong>救不回</strong>"
            "——要修就得修地基。",
            "<strong>The embedding is retrieval's foundation</strong>: a chunk missed at recall <strong>can't be "
            "recovered</strong> by any clever rerank / synthesis — to fix it you must fix the foundation.",
        ), kind="note"),
        L(
            ...
            "</strong>、且根因是<strong>领域语义错位</strong>时，才轮到微调。",
            "L21's rerank can only reorder <strong>already-retrieved candidates</strong>, and "
            ...
            "does fine-tuning's turn arrive.",
        ),
    )
    + c.section(
        ...
        L(
            "把文档切成块（你早在 L06 就会），对<strong>每个块</strong>让一个 LLM <strong>反向生成</strong>"
            ...
            "出一个<strong>可直接用于微调的 embedding QA 数据集</strong>。",
            "Chunk the docs (you've done this since L06), and for <strong>each chunk</strong> have an "
            ...
            "does: feed in nodes, get back an <strong>embedding QA dataset ready for fine-tuning</strong>.",
        ),
        L(
            "要清醒的是：这些问题是<strong>机器造的</strong>，质量"
    ...
    + c.section(
        ...
        L(
            ...
            "<code>generate_qa_embedding_pairs(nodes, llm=...)</code> auto-produces the QA dataset (see (2)).",
        ),
        L(
            ...
            "<code>get_finetuned_model()</code> returns a <strong>domain version</strong> of the embedding.",
        ),
        c.alert(L(
            "<strong>换 embedding 必须重建索引</strong>：旧索引里的向量是<strong>旧模型</strong>算的、<strong>不能和新模型"
            "混用</strong>——换模型就要把整库重新 embedding 一遍。",
            "<strong>Swap the embedding, rebuild the index</strong>: the old index's vectors were computed by the "
            "<strong>old model</strong> and <strong>can't be mixed with the new one</strong> — a new model means "
            "re-embedding the whole corpus.",
        ), kind="key"),
        L(
            "③ <strong>换模型重建索引</strong>——<strong>最容易被忽略、却必须做</strong>的一步：必须把微调模型设成 "
            "<code>Settings.embed_model</code>、<strong>重新 embedding 整库、重建索引</strong>（这也提醒你微调有多贵——"
            "要重跑一遍建库）。",
            "(3) "
            "<strong>Rebuild the index with the new model</strong> — the <strong>easily-missed but mandatory</strong> "
            "step: set the fine-tuned model as "
            "<code>Settings.embed_model</code> and <strong>re-embed the whole corpus and rebuild the index</strong> (a "
            "reminder of how costly fine-tuning is — you re-run the build).",
        ),
        L(
            "④ <strong>量化提升"
            ...
            "的信号。",
            "(4) <strong>Quantify the gain</strong> — the "
    ...
    + c.section(
        ...
        L(
            ...
            "“domain semantics” and get <strong>worse</strong> on real questions.",
        ),
        c.alert(L(
            "<strong>微调是最贵的最后一招</strong>：要造数据、训 GPU、重建整库、还易过拟合——先穷尽更便宜、更稳的手段。",
            "<strong>Fine-tuning is the most expensive last resort</strong>: building data, GPU training, rebuilding the "
            "whole index, and overfit-prone — exhaust the cheaper, steadier levers first.",
        ), kind="warn"),
        L(
            "所以在动微调之前，<strong>先把这串更便宜、风险更低的手段穷尽</strong>：① <strong>调 chunking（L06）"
            "</strong>——块太大太小都稀释语义，先把切块切对；② <strong>加 metadata（L07）</strong>——把标题 / 章节 / 关键词喂给"
            "检索，常常立竿见影；",
            "So before touching fine-tuning, "
            ...
            "— feed titles / sections / keywords to retrieval, often an instant win;",
        ),
        L(
            "③ <strong>上 hybrid + rerank（L21）</strong>——向量召回配上关键词（BM25）兜住术语的精确匹配，"
    ...
    + c.design_highlight(
        i18n.render(L(
            ...
            "检索</strong>更准，和 L01 那条主线<strong>始终一致</strong>。",
            ...
            "fully <strong>consistent</strong> with L01's through-line.",
        ))
        + i18n.render(L(
            "整本书其实在反复讲<strong>同一个判断</strong>：<strong>"
            ...
            "量化</strong>，正是这套手艺的纪律。",
            "The whole book has, in truth, repeated <strong>one "
            ...
            "<strong>quantify at the start, quantify at the end</strong> is the discipline of this craft.",
        ))
        + i18n.render(L(
            "三十五课走下来，你已经把 RAG 从<strong>“能跑一次”</strong>搭成了一条<strong>可检索、"
