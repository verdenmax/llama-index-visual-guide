...
QUIZZES = {
    ...
    "12-retrievers.html": {
        ...
        "interview": [
            ...
            {"q": L("<code>retrieve()</code> 返回的是什么类型？此刻 LLM 调用了吗？",
                    "What type does <code>retrieve()</code> return, and has the LLM been called yet?"),
             "answer": L("返回 <code>List[NodeWithScore]</code>（Node + 相似度分数）。此刻<strong>还没</strong>调用 LLM——检索是查询路径的第一步，生成在后面的合成阶段。",
                         "A <code>List[NodeWithScore]</code> (Node + similarity score). The LLM has <strong>not</strong> been called yet — retrieval is the first step of the query path; generation happens later in synthesis.")},
    ...
    "17-settings-prompts.html": {
        ...
        "interview": [
            {"q": L("<code>Settings</code> 取代了什么？“全局默认 + 局部覆盖”是怎么配合的？",
                    "What did <code>Settings</code> replace, and how do 'global defaults + local override' work together?"),
             "answer": L("取代了旧的 ServiceContext，是个全局单例，集中配置 llm / embed_model / node_parser / chunk_size 等，去样板、可审计。局部可覆盖：某个 query engine 传自己的 <code>llm=</code> 或 <code>text_qa_template=</code> 即可，不动全局。",
                         "It replaced the old ServiceContext — a global singleton centralizing llm / embed_model / node_parser / chunk_size, cutting boilerplate and easing audit. Local override: a given query engine can pass its own <code>llm=</code> or <code>text_qa_template=</code> without touching the global.")},
    ...
    "18-advanced-retrieval.html": {
        ...
        "interview": [
            ...
            {"q": L("Query Fusion 是怎么提升召回的？它的额外成本是什么？",
             ...
             "answer": L("把一个问题改写成<strong>多个</strong>版本（<code>num_queries</code>）各自检索，再用 reciprocal rerank 融合多路结果——覆盖更多“问法”，召回更全。代价：多跑几轮检索 + 一次改写的 LLM 调用，更慢、更贵。",
                         "It rewrites one question into <strong>several</strong> variants (<code>num_queries</code>), retrieves each, then fuses the result lists via reciprocal rerank — covering more phrasings for fuller recall. Cost: several extra retrieval passes plus a rewrite LLM call — slower and pricier.")},
